...
def parse_args():
    parser = argparse.ArgumentParser(description='Train LBCE-BERT model with MLP')
    parser.add_argument(
...
def main():
    ...
    if n != len(labels) or n != embeddings.shape[0]:
        sys.exit(f"Error: sequence count ({n}) != labels ({len(labels)}) or embeddings rows ({embeddings.shape[0]})")
    ...
    model_file = os.path.join(out_dir, 'mlp_model.pt')
